...
from config import anchorinlocalgrid, anchorinlocalgrid_strided
from config import anchorangles, anchorangles_strided
from config import anchornangles, anchornangles_strided
...
anchorcenterpoints = (anchorinlocalgrid_strided - localgridlen//2 -
...
@nb.njit(nb.f8(nb.b1[:,:,:], nb.b1[:,:,:], nb.i8, nb.i8, nb.i8,
               nb.i8[:,:,:], nb.f8[:,:], nb.i8, nb.f8))
def useBoostedTree3(grid, roughgrid, anchorx, anchory, direction,
                   btsplits, btleaves, ntrees, score):
...
@nb.njit(nb.i8(nb.f8[:,:], nb.i8[:], nb.f8[:,:,:,:], nb.i8[:,:,:], nb.f8[:,:],
                 nb.f8[:,:], nb.b1[:,:,:,:], nb.f8[:], nb.i8))
def predictNegs(pts, tileidxs, groundTs, btsplits, btleaves,
            pts2suppress, detections, scores, detectioncount):
    ...
    guysforotherangle = np.zeros((36,2))
        ...
        for angle in range(0, anchornangles, 2):
            ...
            for anchoridx in range(nanchors_strided):
                ...
                for pt2suppressidx in range(npts2suppress):
                    ...
                    if (np.hypot(ptx-anchorcenterptx, pty-anchorcenterpty) < 1.75 and
                             abs(ptcos*angsin - ptsin*angcos) < .7):
                        suppressed = True
                ...
                score1 = useBoostedTree2(thisgrid, roughgrid, anchorx, anchory, 0,
                                        btsplits, btleaves, 10)
                score2 = useBoostedTree2(thisgrid, roughgrid, anchorx, anchory, 1,
                                        btsplits, btleaves, 10)
                if score1 > -30 or score2 > -30:
                    guysforotherangle[anchoridx,0] = score1
                    guysforotherangle[anchoridx,1] = score2
#                    bestlocalscore = -30
#                    bestlocalanchorx = 0
#                    bestlocalanchory = 0
#                    bestlocaldirection = 0
                    for anchorx2 in range(anchorx-1, anchorx+3):
                        for anchory2 in range(anchory-1, anchory+3):
                            score12 = useBoostedTree3(thisgrid, roughgrid, anchorx2,
                                            anchory2, 0, btsplits, btleaves, ntrees, score1)
#                            if score12 > bestlocalscore:
#                                bestlocalscore = score12
#                                bestlocalanchorx = anchorx2
#                                bestlocalanchory = anchory2
#                                bestlocaldirection = 0
                            score22 = useBoostedTree3(thisgrid, roughgrid, anchorx2,
                                            anchory2, 1, btsplits, btleaves, ntrees, score2)
#                            if score22 > bestlocalscore:
#                                bestlocalscore = score22
#                                bestlocalanchorx = anchorx2
#                                bestlocalanchory = anchory2
#                                bestlocaldirection = 1
                            if score12 > score22:
                                bestlocalscore = score12
                                bestbigscore = score1
                                direction = 0
                                ...
                                bestbigscore = score2
                                direction = 1
                            if bestlocalscore > -30: # otherwise, consider culled
                                ...
                                if detectionidx < ndetections:
                                    sample = thisgrid[anchorx2:anchorx2+anchorlen[0],
                                                      anchory2:anchory2+anchorlen[1], :]
                                    if direction:
                                        sample = sample[::-1,::-1]
                                    detections[detectionidx] = sample
                                    scores[detectionidx] = bestbigscore
                                detectioncount += 1
            
            thisgrid = grid[angle+1]
            prepRough(thisgrid, roughgrid)
            for anchoridx in range(nanchors_strided):
                anchorx, anchory = anchorinlocalgrid_strided[anchoridx]
                score1, score2 = guysforotherangle[anchoridx]
                if score1 > -30 or score2 > -30:
#                    bestlocalscore = -30
#                    bestlocalanchorx = 0
#                    bestlocalanchory = 0
...
if __name__ == '__main__':
    ...
    BT_load_file = '../dataApril19/BT510.npz'
    #np.random.seed(200)
    nnegatives = 715
    nfilesfornegatives = 6
    ...
    for file_idx in files2use:
        ...
        detectioncount = predictNegs(pts, tileidxs, groundTs, btsplits, btleaves,
                                     pts2suppress, globaldetections, detectioncount)
